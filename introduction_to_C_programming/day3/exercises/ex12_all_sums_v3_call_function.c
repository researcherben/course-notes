...
int call_me(int old_result, int old_indx){
  int result;
  for (int indx=1; indx<old_result/2.0; indx++){
    result=old_result-indx;
    if (!(old_indx==indx) && !(old_indx==result))
      printf("%d+%d+%d\n",result,indx,old_indx);
  }
}
int main() {
